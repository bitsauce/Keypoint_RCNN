...
def unmold_kp_mask(kp_masks, bbox, image_shape):
    """Converts a mask generated by the neural network to a format similar
    to its original shape.
    kp_masks: [NUM_KEYPOINTS, height, width] of type float. A small, typically 28x28 mask.
    bbox: [y1, x1, y2, x2]. The box to fit the mask in.
    ...
    for i in range(kp_masks.shape[0]):
        ...
        full_mask = np.zeros(image_shape[:2], dtype=kp_masks.dtype)
        full_mask[y1:y2, x1:x2] = kp_mask
        full_masks.append(full_mask)
    return np.array(full_masks)
